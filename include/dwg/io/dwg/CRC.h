...
public:
    ...
    static unsigned short ApplyCrc8(unsigned short dx, const std::vector<unsigned char>& arr)
    {
		...
		{
			unsigned char al = (unsigned char)(arr[i] ^ (unsigned int)(unsigned char)(retval & (unsigned int)UCHAR_MAX));
			retval = (unsigned short)((unsigned short)(retval >> 8 & UCHAR_MAX) ^ (unsigned int)CrcTable[al & UCHAR_MAX]);
		}
		return retval;
    }
};
