...
public:
    ...
    {
        if (date < 1721426 || date > 5373484) { return DateTime(); }
    ...
    static void DateToJulian(DateTime date, int &jdate, int &miliseconds)
    {
        ...
        // int month = date.Month;
        // jdate = date.Day +
        //         (int) System.Math.Floor(
        //                 (153.0 * (double) (month + 12 * day - 3) + 2.0) / 5.0) +
        //         365 * year + (int) System.Math.Floor((double) year / 4.0) -
        //         (int) System.Math.Floor((double) year / 100.0) +
        //         (int) System.Math.Floor((double) year / 400.0) - 32045;
