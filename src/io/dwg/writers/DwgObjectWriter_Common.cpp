...
void DwgObjectWriter::writeCommonData(CadObject *cadObject) 
{
			...
			switch (cadObject.ObjectType)
			{
				case ACadVersion::UNLISTED:
					...
					{
						notify($"Dxf Class not found for {cadObject.ObjectType} fullname: {cadObject.GetType().FullName}", Notification::Warning);
						return;
					}
					break;
				case ACadVersion::INVALID:
				case ACadVersion::UNDEFINED:
					notify($"CadObject type: {cadObject.ObjectType} fullname: {cadObject.GetType().FullName}", Notification::NotImplemented);
					return;
				default:
				...
				foreach (ExtendedDataRecord record in entry.Records)
				{
					...
					switch (record)
					{
						...
						case ExtendedDataString str:
							...
							{
								unsigned char[] bytes = _writer->Encoding.GetBytes(string.IsNullOrEmpty(str.Value) ? string.Empty : str.Value);
								mstream.Write(LittleEndianConverter.Instance.GetBytes((ushort)str.Value.Length + 1), 0, 2);
								mstream.Write(bytes, 0, bytes.Length);
								mstream.WriteByte(0);
							}
