#include "cptable932.h"
#include "cptable936.h"
#include "cptable949.h"
#include "cptable950.h"
#include "cptables.h"
#include <algorithm>
#include <cstring>
#include <dwg/utils/TextCodec.h>
#include <iconv.h>
#include <iomanip>
#include <sstream>
...
TextCodec::~TextCodec() { delete conv; }

void TextCodec::setVersion(int v, bool dxfFormat)
{
    ...
    {
        version = DRW::AC1021;
        if (dxfFormat) cp = "UTF-8";//RLZ: can be UCS2 or UTF-16 16bits per char
        else
...
void TextCodec::setVersion(std::string *v, bool dxfFormat)
{
    ...
    else { setVersion(DRW::AC1021, dxfFormat); }
}

void TextCodec::setCodePage(std::string *c, bool dxfFormat)
{
    ...
    if (version == DRW::AC1009 || version == DRW::AC1015)
    {
        ...
        else { conv = new DRW_ExtConverter("SJIS"); }
    }
    else
    {
        if (min_ver <= DRW::AC1018) { conv = new DRW_ExtConverter("SJIS"); }
        else
        {
            if (dxfFormat) conv = new DRW_Converter(NULL, 0);//utf16 to utf8
            else
...
std::string TextCodec::toUtf8(std::string s) { return conv->toUtf8(&s); }

std::string TextCodec::fromUtf8(std::string s) { return conv->fromUtf8(&s); }
        ...
        {//2 bytes
            ...
            if (end > 0)
            {
                for (int k = sta; k < end; k++)
                {
                    ...
                    {
                        res += encodeNum(
                                DRW_DoubleTable932[k][1]);//translate from table
                        notFound = false;
                        break;
                    }
                }
