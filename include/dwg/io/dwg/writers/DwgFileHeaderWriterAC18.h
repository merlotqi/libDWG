...
public:
                    ...
                    bool isCompressed, int decompsize = 0x7400) override
    {
        ...
        int spearBytes =
                (int) (stream.Length % (int) descriptor.DecompressedSize);
        if (spearBytes > 0 &&
...
protected:
                                    ...
                                    bool isCompressed)
    {
        ...
        localMap.ODA = DwgCheckSumCalculator.Calculate(
                0u, descriptorStream.GetBuffer(), 0,
        ...
        _stream.Write(checkSumStream.GetBuffer(), 0,
                      (int) checkSumStream.Length);
                     ...
                     int totalSize, bool isCompressed)
    {
        ...
        if (isCompressed)
        {
            ...
            compressor.Compress(holder.GetBuffer(), 0, decompressedSize,
                                stream);
        }
        else
        {
...
public:
    void writeRecords()
    {
        ...
        int size =
                counter + DwgCheckSumCalculator.CompressionCalculator(counter);
        section.Size = size;
        ...
        DwgLocalSectionMap last =
                _localSectionsMaps[_localSectionsMaps.Count - 1];
...
private:
    void writeFileMetaData()
    {
        ...
        _stream.Position = 0L;
        _stream.Write(Encoding.ASCII.GetBytes(_document.Header.VersionString),
                      0, 6);
        ...
        writer.Write(
                (uint) ((int) _descriptors[DwgSectionDefinition.SummaryInfo]
                                .LocalSections[0]
                                .Seeker +
                        0x20));
    ...
    void writeFileHeader(MemoryStream stream)
    {
        ...
        crcStream.Write(TextEncoding.Windows1252().GetBytes("AcFssFcAJMB"), 0,
                        11);
    ...
    void compressChecksum(DwgLocalSectionMap section, MemoryStream stream)
    {
        ...
        section.Checksum = DwgCheckSumCalculator.Calculate(
                (uint) section.Checksum, main.GetBuffer(), 0,
                (int) main.Length);
