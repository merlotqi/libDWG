...
public:
    ...
    DwgAuxHeaderWriter(std::ostringstream* stream, Encoding encoding, const CadHeader& header) : DwgSectionIO(header.Version)
    {
        _stream = stream;
        _encoding = encoding;
        _header = header;

        _writer = DwgStreamWriterBase::GetStreamWriter(_version, _stream, Encoding::Unicode);
    }
