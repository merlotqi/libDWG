...
public:
    size_t HandleSectionOffset() const;
    {
        size_t offset = _fileHeaderSize;

        foreach (auto &&item : _records)
        {
    ...
    {
        this._records[name].first.Size = ostream_length(stream);
        this._records[name] = {_records[name].first, stream};
    }
